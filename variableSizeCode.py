...
# given 2 input files, encode and save to the output file
def encodeDataFromData(inputStr, markovInputFile, textFileFormat, wordsPerState = 1):
	initTime = time.time()
...
# decode from file
def decodeDataFromData(inputData, markovInputFile, wordsPerState = 1):
	initTime = time.time()
	...
	print "read " + repr(decodedData.totalFieldLen()) + " bits"
	decodedData = decodedData.getAllBytes()
	decodedStr = "".join(str(chr(x)) for x in decodedData)
	endTimeDecode = time.time()
